...
void CallBackFunc(int event, int x, int y, int flags, void* userdata)
{
	...
	double depth =  4.0*result + 3.0;

	double focal = 400; //calculate focal length and assign it here
	...
	if (event == EVENT_LBUTTONDOWN)
	{
		...
		cout << "Left button of the mouse is clicked - position (" << x << ", " << y << "), depth (data value " << depth_one << ")" << endl;
	...
	else if (event == EVENT_RBUTTONDOWN)
	{
		...
		y_two = y;
		depth_two = depth;
		cout << "Right button of the mouse is clicked - position (" << x << ", " << y << "), depth (data value " << depth_two << ")" << endl;
	}
	else if (event == EVENT_MBUTTONDOWN)
	{
		dis = sqrt((x_two - x_one)*(x_two - x_one) + (y_two - y_one)*(y_two - y_one));
		width_cm = (dis*depth_two) / focal;
		cout << "Distance = " << dis << endl;
		cout << "Width 2D (cm) = " << width_cm << endl;
		
		double Dist1 = dis;
		double Dist2 = sqrt(((((Dist1*depth_two) / focal))*((Dist1*depth_two) / focal)) + (depth_two - depth_one)*(depth_two - depth_one));
		cout << "Width 3D (cm) = " << Dist2 << endl;
	}
